...
기본 프롬프트가 없는 순수 API 요청 모드입니다. 특정한 제약 없이 자유롭게 대화할 수 있습니다.
...
**사용 예시**: 급하게 쓴 메모를 정돈된 문장으로 변환, 간단한 질문, 아이디어 브레인스토밍

### 2. 유사어 분석

단어와 표현의 유사어를 분석하고 뉘앙스 차이를 설명합니다. 문맥에 맞는 적절한 대체어를 찾아드립니다.
...
**사용 예시**: 번역 시 적절한 단어 선택, 글쓰기에서 반복 표현 회피, 정확한 어감 전달

**입력 형식**: `분석할 단어나 표현을 입력해주세요`
...
**사용 예시**: 독서 기록 작성, 책 리뷰 개선, 논리적 글쓰기 연습

**입력 형식**: `비평받고 싶은 글을 입력해주세요`
...
**사용 예시**: 시사 정보 조사, 트렌드 파악, 특정 이슈에 대한 다각도 분석

**입력 형식**: `궁금한 주제나 키워드를 입력해주세요`
...
**사용 예시**: 학술 조사, 기술 리서치, 복잡한 개념 이해, 전문 분야 심층 분석

**입력 형식**: `탐구하고 싶은 주제를 입력해주세요`

### 6. XAI 프롬프트 엔지니어

Grok(XAI) 모델에 최적화된 프롬프트를 생성합니다. XAI 모델의 특성을 고려한 효과적인 프롬프트를 작성해드립니다.
...
**사용 예시**: Grok API 활용 시 프롬프트 작성, XAI 기반 서비스 개발, 프롬프트 최적화 학습

**입력 형식**: `프롬프트 엔지니어링이 필요한 작업을 입력해주세요`

### 7. OpenAI 프롬프트 엔지니어

ChatGPT(OpenAI) 모델에 최적화된 프롬프트를 생성합니다. GPT 시리즈의 강점을 최대한 활용할 수 있는 프롬프트를 작성해드립니다.
...
**사용 예시**: GPT-4 API 활용 시 프롬프트 작성, OpenAI 기반 챗봇 개발, 프롬프트 테스팅

**입력 형식**: `프롬프트 엔지니어링이 필요한 작업을 입력해주세요`

### 8. 작품 스토리 분석

소설, 영화, 드라마 등의 스토리 구조와 캐릭터를 분석합니다. 서사 구조, 주제 의식, 캐릭터 아크를 심층적으로 분석합니다.
...
**사용 예시**: 창작 피드백, 시나리오 리뷰, 문학 작품 분석, 스토리텔링 학습

**입력 형식**: `분석할 작품명을 입력해주세요`

### 9. 업체 요구사항 분석

클라이언트 요구사항을 정리하고 개발 스펙으로 변환합니다. 모호한 요청을 구체적인 기능 명세로 전환합니다.
...
**사용 예시**: 프로젝트 기획, 요구사항 명세서 작성, 클라이언트 미팅 준비, 개발 범위 정의

**입력 형식**: `분석할 업체 요구사항을 입력해주세요`

### 10. 번역본 비교

동일 원문의 여러 번역본을 비교하고 차이점을 분석합니다. 번역 선택지와 뉘앙스 차이를 상세히 설명합니다.
...
**사용 예시**: 번역 품질 검토, 문학 번역 연구, 언어 학습, 번역서 선택 가이드

**입력 형식**: `작가명 - 책 제목을 입력해주세요 (예: 헤르만 헤세 - 데미안)`

---

### 서비스 전환 방법

[ChatHeader.vue](src/components/widgets/ChatHeader.vue)의 드롭다운 메뉴에서 서비스를 선택할 수 있습니다. 서비스를 전환하면:

1. 입력 필드의 placeholder가 해당 서비스에 맞게 자동 변경됩니다
2. 인트로 메시지가 표시되어 서비스의 주요 기능을 안내합니다
3. 이전 대화는 유지되며, 새로운 서비스로 계속 대화할 수 있습니다

각 서비스는 독립적인 프롬프트 설정을 가지고 있어, 같은 질문이라도 서비스에 따라 다른 관점의 답변을 받을 수 있습니다.
